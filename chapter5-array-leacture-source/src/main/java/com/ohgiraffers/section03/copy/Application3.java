...
public class Application3 {
    public static void main(String[] args) {
        ...
        * 서로 같은 값ㅅ을 가지고 있지만, 두 배열은 서로 다른 배열이기에
        * 하나의 배열에 변경을 하더라ㅣㄷ 다른 배열에는 영향을 주지 안흔다
        * */

        /*
        * 깊은 복사를 하는 방법은 4가지가 있다
        * 1. for문을 이용한 동일 인덱스 값 복사
        * 2. Object의 clone을 이요한 복사
        * 3. System의 arraycopy()를 이용한 복사
        * 4. Arrays의 copyOf()를 이용한 복사
        * 이중 가장 높은 성능 보이는 것은 순수 배열의 복사를 위해 만들어진 arraycopy() 매ㅔ소드 이며
        * 가장 많이 사용되는 바식은 좀 더 유명한 방식인 copyOf()메소드이다
        * clone()의 경우 이전 배열과 같은 배열박에 만들 수 없다는 특징을 갖는다
        * */
